...
# before filling the variable, we will will denote this one unfilled
d_analysis_selected = d_analysis_selected %>% mutate(inj_knee_unfilled = inj_knee_filled,
                                                     knee_total_unfilled = knee_total_filled)

# imputed missing outcome data - for now. This is only temporary.
# we will return the missing values later.
d_strata = d_analysis_selected %>% group_by(d_imp, id_player, season) %>% 
  fill(inj_knee_filled, knee_total_filled, .direction = "downup") 
...
d_surv = d_surv %>% group_by(d_imp, id_player, season) %>% 
  mutate(knee_total_filled_lag = lag(knee_total_filled),
         ...
         status = ifelse(knee_total_diff > 0 & knee_total_filled_lag != 0, 1, 0),
...
d_weekly = d_surv %>%
  dplyr::select(d_imp, all_of(key_cols), 
                ...
                status, inj_knee_filled, prev_symptoms, knee_total_unfilled,
...
write_excel_csv(fit_pooled, paste0("symptomatic_to_worse_ewma.csv"), delim = ";", na = "")
...
png("figure3_predicted_probs_worsening.png", width = 12, height = 4, unit = "in", res = 600)
ggpubr::ggarrange(plot_load, plot_weeks, labels = "AUTO")
dev.off()
