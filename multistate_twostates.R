...
statenames = c("asymptomatic", "symptomatic")
...
d_selected = d_selected %>% 
  mutate(diff_prev = inj_knee_filled-lag(inj_knee_filled),
         change_missing = ifelse(!is.na(inj_knee_filled) & is.na(diff_prev), 1, 0),
...
d_surv = d_surv %>% mutate(knee_state = ifelse(inj_knee_filled == 1, 2, 1))
...
d_from1 = d_filled  %>% 
  ...
  ungroup() %>% 
  pivot_longer(cols = c("trans1"), names_to = "trans", values_to = "status") %>% 
  mutate(trans = str_replace_all(trans, "trans", ""),
         to = 2,
         id_state = 1)

d_from1 = d_from1 %>% add_event_id(status)
...
d_from2 = d_from2 %>% add_event_id(status)

# bind data
# congratulations, they are now ready for analysis
d_multistate = bind_rows(d_from1, d_from2) %>% arrange(d_imp, id_player, date)
